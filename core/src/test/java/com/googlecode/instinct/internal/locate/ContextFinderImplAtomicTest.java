        ...
        expect.that(new Expectations() {
            {
                ...
                one(objectFactory).create(with(same(File.class)), with(same(PACKAGE_ROOT))); will(returnValue(packageRoot));
                expectClassFileFilterCreated();
                expectMethodFileFilterCreated();
                one(classLocator).locate(packageRoot, classFileFilter, methodFileFilter); will(returnValue(classNames));
            }
            ...
            private void expectClassFileFilterCreated() {
                ...
                one(objectFactory).create(
                        with(same(MarkingSchemeImpl.class)), with(same(Context.class)), with(same(contextNamingConvention)), with(equal(attributeConstraint)));
                will(returnValue(contextMarkingScheme));
                one(objectFactory).create(with(same(ClassWithContextAnnotationFileFilter.class)), with(same(packageRoot)), with(same(contextMarkingScheme)));
            ...
            private void expectMethodFileFilterCreated() {
                ...
                one(objectFactory).create(
                        with(equal(MarkingSchemeImpl.class)), with(equal(Specification.class)), with(equal(specificationNamingConvention)), with(equal(attributeConstraint)));
                will(returnValue(specificationMarkingScheme));
                one(objectFactory).create(with(same(ClassWithMarkedMethodsFileFilter.class)), with(same(packageRoot)), with(same(specificationMarkingScheme)));
